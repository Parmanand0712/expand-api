...
exports.getOraclePrice = async (req, res) => {
  const time = new Date().getTime();
  try {
    const price = await helper.oracleCommon("getPrice", req.query);
    if (
...
exports.getOracleSupportedTokens = async (req, res) => {
  const time = new Date().getTime();
  try {
    const price = await helper.oracleCommon("getSupportedTokens", req.query);
    if (
      price.valid !== undefined ||
      price.error !== undefined ||
      price.message !== undefined ||
